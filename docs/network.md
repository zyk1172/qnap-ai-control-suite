...
QNAP Virtual Switch 的私有命令或本机 API 不同版本差异较大，先通过 `nas_discovery` 和 `scripts/qnap_probe.sh` 确认实际 utilities。将已验证 argv 写入 `qnap_adapters.virtual_switch` 后使用 `nas_virtual_switch_action`；在 adapter 尚未配置时，再通过 `nas_exec` 执行对应的已验证命令。
