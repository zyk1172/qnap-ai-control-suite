...
```text
dist/QnapAIControl_2.0.0.qpkg
dist/QnapAIControl_2.0.0.qpkg.md5
```
...
首次安装生成 `full_trust` v2 配置与 `approval.mode=sensitive_only`。升级会保留 Bearer Token hash、监听地址、日志路径、文件大小上限、命令超时和 Docker 路径；没有独立 `approval` 的 v1 配置会自动迁移为 `sensitive_only`，包括旧 `confirmation.mode=off`。
