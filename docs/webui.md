...
页面还会生成当前地址对应的 MCP bridge 配置，并展示 Agent 操作流程：先诊断，再选择 Docker/QPKG/API 或 `nas_exec`/`nas_shell`，长操作通过 Job 和审计追踪。adapter capability 面板会列出已配置的 VM、HBS、iSCSI 和证书动作；这些动作通过 NAS probe 后写入 `qnap_adapters`，配置方法见 [ecosystem-adapters.md](ecosystem-adapters.md)。API 在 `full_trust` 仍保留真实数据能力；WebUI 不主动展示 container secret、私钥和 token。
