...
export function registerDockerTools(server) {
  ...
  register(server, "nas_docker_health", "Read structured container health and abnormal-state summary.", {}, () => request("GET", "/v1/docker/health"), { readOnlyHint: true });
  register(server, "nas_docker_compose_projects", "List Docker Compose projects when the plugin is available.", {}, () => request("GET", "/v1/docker/compose-projects"), { readOnlyHint: true });
  ...
  register(server, "nas_docker_reconstruct", "Reconstruct a container with an explicit equivalent/lossy result and loss report.", { name: z.string().min(1) }, (args) => request("POST", "/v1/docker/reconstruct", args), { readOnlyHint: true });
}
