...
export function registerSystemTools(server) {
  ...
  register(server, "nas_capabilities", "Read both the QACS control-plane policy and the actual QNAP ecosystem capability state. Use qnap.adapters[].capability_states to decide which structured actions are available, degraded, or unavailable before selecting a QNAP tool.", {}, async () => {
    const [control, qnap] = await Promise.all([
      request("GET", "/v1/capabilities"),
      request("GET", "/v1/qnap/ecosystem")
    ]);
    return { control, qnap };
  }, { readOnlyHint: true });
  ...
  register(server, "nas_qnap_probe", "Run the QPKG-bundled read-only QNAP runtime probe and write JSON to an absolute NAS path. This is a diagnostic/development fallback; normal Agent use should prefer nas_capabilities and structured tools.", { output_path: z.string().min(1), dry_run: z.boolean().optional() }, (args) => request("POST", "/v1/qnap/probe", args), { destructiveHint: true });
