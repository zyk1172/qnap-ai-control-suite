...
import { longRequestTimeoutMs } from "../config.js";
...
export function registerJobTools(server) {
  register(server, "nas_job_start", "Start an asynchronous arbitrary argv command or shell script. Use for long Docker, backup, archive, storage, firmware, or QNAP commands; poll nas_job_get and nas_job_logs.", jobStart, (args) => request("POST", "/v1/jobs", args, { timeoutMs: longRequestTimeoutMs }), { destructiveHint: true });
